...
class Config:
    ...
    PLATFORM_DEVICE_NAME = '新大陆Modbus网关'
    PLATFORM_LOCATION = '默认监测点'
    ...
    PLATFORM_GATEWAY_URL = 'http://127.0.0.1:8000/api/gateway/poll'
    PLATFORM_SYNC_TIMEOUT = 2
...
class PlatformBackendClient:
    ...
    def __init__(self, gateway_url: str, timeout: float):
        self.gateway_url = gateway_url
        self.timeout = timeout

    def upload_registers(self, cfg: Config, r0: int, r1: int) -> bool:
        payload = {
            'device_id': cfg.PLATFORM_DEVICE_ID,
            'device_name': cfg.PLATFORM_DEVICE_NAME,
            'location': cfg.PLATFORM_LOCATION,
            'sample_registers': {
                'R0': r0,
                'R1': r1,
            },
        ...
        try:
            response = requests.post(self.gateway_url, json=payload, timeout=self.timeout)
            response.raise_for_status()
            result = response.json()
            logger.info(
                f"[平台后端] 同步成功: mode={result.get('mode')}, device={payload['device_id']}"
            )
            return True
        except Exception as e:
            logger.warning(f"[平台后端] 同步失败: {e}")
            return False
...
def main():
    ...
    platform_client: Optional[PlatformBackendClient] = None
    if cfg.PLATFORM_SYNC_ENABLED:
        platform_client = PlatformBackendClient(cfg.PLATFORM_GATEWAY_URL, cfg.PLATFORM_SYNC_TIMEOUT)
    ...
    while True:
        try:
            ...
            if values and len(values) >= 2:
                ...
                if platform_client:
                    platform_client.upload_registers(cfg, r0, r1)
            else:
